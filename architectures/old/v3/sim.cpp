...
struct Computer {
    ...
    Computer() {
        program = vector<uint16_t>(256, 0);
        ram = vector<uint8_t>(256, 0);
        stack = vector<uint16_t>(16, 0);
        reset();
    }
    ...
    Op decode(uint16_t instruction) {
    ...
    void loadProgram(vector<uint16_t>& programCode) {
    ...
    uint8_t ret(uint16_t instruction) {
        sp--;
